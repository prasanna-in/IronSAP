...
class fingerprint:
  ...
  def loginbypass(self,session,username):
    ...
    if val==0:
      ...
      pr.Summary = "SAP Web Login bypass could be possible with Header Inclusion attack -- SAP Active Directoe=ry Authtenction in use."
      pr.Plugin = " "
      ...
      self.isap.print_out("Remote Authentication Dint Work."+"\n", 1)
        ...
        for url in urlist:
          if requrl==url:
            ...
            if requrl=="/sap/admin/public/index.html":
              ...
              pr.Summary = "Web Administration Interface could leaking some sensitive information."
              pr.Plugin = " "
    ...
    if len(self.page401list)>0:
      ...
      for req in self.page401list:
        ...
        if resp.Code==200:
          ...
          pr.Summary = "Resources that are protected with login could be accessed by changing the HTPP Request Method."
          pr.Plugin = " "
  ...
  def starts(self):
    httpports = []
    f = open("ports.txt","a")
    self.isap.print_out("HTTP Ports Detected Are:"+"\n", 0)
